...
var bullet_scene = preload("res://assets/bullets/MustardMachineGunBullet.png")

# Define exported variables with type hints
#@export var damage: int = 10
#@export var fire_rate: float = 2.0
@export var type: int = WeaponTypes.WeaponType.MUSTARD_MACHINE_GUN
...
func shoot():
	const BULLET = preload("res://MustardMachineGunBullet.tscn")
	var enemies_in_range = get_overlapping_bodies()
	if enemies_in_range.size() > 0:
